# ARISTA-CVASS-SDK
A Python SDK for interacting with Arista's Cloud Vision as a Service (CVAAS). This SDK simplifies the management, monitoring, and automation of network services within the Arista ecosystem through a structured, object-oriented approach.

### PATH TO AristaCVAAS IF USED IN ANOTHER FOLDER
...
```python
def repace_name(old_names:list,new_name,deliminator="_"):
    result = []
    for x in old_names:
        split = x.split(deliminator)
        # print(split)
        split[0]= new_name
        result.append('_'.join(split))
    return result
```
...
server_url = input(prompt="API URL:")

api = AristaCVAAS(server_url, token)
```

    API Key: exampleapikey1234556677
    API URL: https://example_url_for_api_access/cvpservice
...
# TEST
try:
    # Get all configlet names and IDs
    cvp_info = api.get_cvp_info()
    pp.pprint(cvp_info)
...
```python
[x[0] for x in api.get_configlet_names_ids()]
```
...
```python
api.get_configlet_names_ids("device")
```
...
```python
api.get_configlet_applied_containers(["global_aaa_conf"])
```
...
```python
api.get_configlet_applied_devices(["device_devfrirt98_hostname_conf"])
```
...
api.get_users_and_groups(filter_status='Online')
```
...
```python
pp.pprint(api.get_inventory_device_config("ba:dd:ea:db:ee:f0"))
```
...
```python
pp.pprint(api.get_users_and_groups())
```
...
# print(api.copy_configlet(["devfricr01_hostname_conf"]))
```
...
```python
# print(api.append_configlet("RECONCILE_ZTW285931A9", "global_aaa_conf"))
```
...
```python
print(api.get_configlet_names_ids(regex="cr"))
```
...
```python
api.get_device_configlets(process_all=True)
```
...
```python
api.get_provisioning_filter_topology(value_pattern=r'^WESTMINSTER.*')
```
...
```python
api.extract_container_ids_by_hierarchy(api.get_provisioning_filter_topology(value_pattern=r'^THE.*'))
```
...
data = [{'data': [{'action': 'add',
           ...
           'toIdType': 'container',
           'toName': 'Tenant'}]}]
# api.post_provisioning_add_temp_actions(data_list = data)
```
...
```python
api.get_provisioning_temp_actions()
```
...
```python
api.post_provisioning_save_temp_actions(data=[])
```
...
```python
# api.delete_provisioning_temp_action_all()
```
...
```python
api.get_provisioning_filter_topology(value_pattern=r'^rightman2.*')
```
...
```python
data = []
for x in api.extract_container_ids_by_hierarchy(api.get_provisioning_filter_topology(value_pattern=r'^campus.*')):
...
# api.post_provisioning_add_temp_actions(data = data)
```
...
hierarchy_dict = api.generate_topology_hierarchy_post_data(input_dict,print_ascii=True)
```
...
```python
api.generate_topology_hierarchy_ascii_tree(value_pattern=r'^WESTMINSTER.*',return_structure=True)
```
...
```python
all_configlet_names = [x[0] for x in api.get_configlet_names_ids()]
all_applied_containers = api.get_configlet_applied_containers(all_configlet_names)
...
names_conf = ['global_aaa_conf',
   ...
   'global_spanning-tree_conf',
   'global_time_conf']
api.get_configlet_ids_by_name(names_conf)
```
...
```python
api.get_container_id_by_name("WESTMINSTER")
```
...
```python
container_id = api.get_container_ids_by_name('SITE')[0]
configlet_id = api.get_configlet_ids_by_name(['site_snmp_conf'])
print(container_id,configlet_id)
post_result = api.post_assign_configlets_to_container(container_id = container_id, configlets= configlet_id)
```
...
```python
api.group_devices(grouping_key = "complianceCode")
```
