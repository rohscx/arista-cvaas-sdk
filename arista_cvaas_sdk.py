...
#     start
    ...
    def assign_configlets_to_devices(
        ...
        *,
        ...
        debug: bool, optional
            When ``True`` prints the generated payloads instead of executing the
            API calls.  The payloads are returned to the caller for inspection.

        Returns
        -------
        List[Union[Dict[str, str], Dict[str, Any]]]
            Aggregated list of responses from
            :meth:`post_provisioning_add_temp_actions`.  When ``debug`` is
            ``True`` the generated payloads are returned instead.
        ...
        * de-duplicating configlets within an assignment.
        ...
        for index, assignment in enumerate(assignments):
            ...
            if not filtered_configlets:
                ...
                if debug:
                    print(f"DEBUG: Skipping assignment {index} for device '{device_id}'"
                          " because no configlets remain after filtering.")
                continue
            ...
            if debug:
                print(f"DEBUG: Prepared payload for assignment {index} (device '{device_id}'):")
        ...
        if debug:
            print("DEBUG: Skipping API execution because debug mode is enabled.")
            return payloads
